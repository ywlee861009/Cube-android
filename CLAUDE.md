...
:app
 ...
 └── assets/
      ...
      └── js/
           ...
           ├── actions.js      — shuffle(), reset(), undo/redo, solve, moveCount 추적
...
MainActivity (Kotlin)
  └── WebView
        └── cube.html
              ...
              ├── actions.js  — 사용자 액션 (shuffle / reset / undo / redo / solve)
...
- `CubingJsSolver`: `cubing-solver.bundle.js`의 `CubingSolver.solveFromFacelets()` 래핑
- `SolverFactory.create()`: 현재 구현체 반환 (교체 시 이 파일만 수정)
- `solveCube()`: 첫 호출 시 솔루션 계산, 이후 호출마다 한 수씩 실행 (step 방식)

**Undo / Redo (actions.js):**
- `undoStack` / `redoStack`: `{ moveName, moveCount }` 엔트리 저장
- `inverseMoveOf(name)`: `U` ↔ `U'` 역이동 변환
- shuffle/solve/undoRedo 진행 중에는 버튼 비활성화 (`isShuffling`, `isSolving`, `isUndoRedo` 플래그)
...
| -  | Undo / Redo (↩/↪ 버튼, 이동 히스토리 스택) | `actions.js`, `cube.html` |
| 5  | cubing.js 기반 솔버 + 팩토리 패턴 | `solver/`, `lib/cubing-solver.bundle.js` |
| 5  | Solve UI: 한 수씩 step 실행 (N/total 표시) | `actions.js`, `cube.html` |
